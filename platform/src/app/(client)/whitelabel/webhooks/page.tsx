import { clientGetWebhooksAction } from "@/actions/client/whitelabel/client-webhooks.action";
import { clientGetBrandingAction } from "@/actions/client/whitelabel/client-get-branding.action";
import { ClientWebhooksView } from "@/components/client/whitelabel/client-webhooks-view";
import {
  Card,
  CardContent,
  CardDescription,
...
export default async function WhiteLabelWebhooksPage() {
  const [webhooksResult, brandingResult] = await Promise.all([
    clientGetWebhooksAction(),
          ...
          <CardHeader className="text-center pb-4">
            ...
            <CardDescription className="text-xs">
              {brandingResult.message ||
                "You must have an approved WhiteLabel application before configuring webhooks."}
            </CardDescription>
          </CardHeader>
  ...
  return (
    <div className="w-full min-h-[calc(100vh-4rem)] p-4 sm:p-6 lg:p-8">
      <ClientWebhooksView
        initialConfig={webhooksResult.webhook || defaultWebhook}
        initialLogs={webhooksResult.logs || []}
