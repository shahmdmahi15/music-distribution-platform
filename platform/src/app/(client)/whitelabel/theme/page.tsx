...
export default async function WhiteLabelThemePage() {
  const [themeResult, brandingResult] = await Promise.all([
    clientGetThemeAction(),
  ...
  const defaultTheme = {
    primaryColor: brandingResult.branding.primaryColor || "#6366f1",
    accentColor: brandingResult.branding.accentColor || "#ec4899",
    radius: "0.5rem",
    mode: "dark" as const,
    fontFamily: "Inter",
  ...
  return (
    <div className="w-full min-h-[calc(100vh-4rem)] p-4 sm:p-6 lg:p-8">
      <ClientThemeView
        initialTheme={themeResult.theme || defaultTheme}
