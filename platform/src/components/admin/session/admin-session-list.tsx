...
import { adminRevokeSessionAction } from "@/actions/admin/session/admin-revoke-session.action";
import { adminRevokeAllOtherSessionsAction } from "@/actions/admin/session/admin-revoke-all-other-sessions.action";
...
export function AdminSessionList({
  ...
  const handleRevokeSingle = async () => {
    ...
    try {
      ...
      const res = await adminRevokeSessionAction(revokingSessionId);
      if (res.success) {
  ...
  const handleRevokeAllOthers = async () => {
    try {
      ...
      const res = await adminRevokeAllOtherSessionsAction();
      if (res.success) {
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.download = `admin_sessions_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const handleExportJSON = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `admin_sessions_${new Date().toISOString().slice(0, 10)}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
