...
import { adminPasswordUpdateAction } from "@/actions/admin/profile/admin-password-update.action";

interface AdminPasswordUpdateCardProps {
  isPasswordLinked: boolean;
}

export function AdminPasswordUpdateCard({
  isPasswordLinked,
}: AdminPasswordUpdateCardProps) {
  ...
  const form = useForm({
    defaultValues: {
      ...
      newPassword: "",
      confirmNewPassword: "",
    } as AdminPasswordUpdateInput,
    validators: {
      onSubmit: adminPasswordUpdateSchema,
    },
    onSubmit: async ({ value, formApi }) => {
      try {
        const result = await adminPasswordUpdateAction(value);
            ...
            Object.entries(result.error.fieldErrors).forEach(
              ([field, messages]) => {
                ...
                if (errorMessages && errorMessages.length > 0) {
                  const fieldName = field as keyof AdminPasswordUpdateInput;
        ...
        console.log("[Component.Admin.Profile.PasswordUpdate] Error: ", error);
  ...
  return isPasswordLinked ? (
    <Card className="shadow-sm border-border/60 flex flex-col justify-between">
      <div>
        ...
        >
          {([isSubmitting, values]) => {
            const currentValues = values as AdminPasswordUpdateInput;
            const isUnchanged =
            ...
            return (
              <>
                ...
                <CardFooter className="pt-2 border-t border-border/40 mt-4 flex justify-end">
                  <Button
                    type="submit"
                    form="admin-password-update-form"
