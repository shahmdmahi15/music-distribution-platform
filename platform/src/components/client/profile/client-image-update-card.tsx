...
import { clientImageUpdateAction } from "@/actions/client/profile/client-image-update-action";
import { clientImageRemoveAction } from "@/actions/client/profile/client-image-remove.action";
import { useRouter } from "next/navigation";

interface ClientImageUpdateCardProps {
  initialImage?: string | null;
  userName?: string;
}

export function ClientImageUpdateCard({
  initialImage = "",
  userName = "Client User",
}: ClientImageUpdateCardProps) {
  ...
  const form = useForm({
    ...
    validators: {
      onSubmit: clientImageUpdateSchema,
    },
    onSubmit: async ({ value, formApi }) => {
      try {
        const result = await clientImageUpdateAction(value);
            ...
            Object.entries(result.error.fieldErrors).forEach(
              ([field, messages]) => {
                ...
                if (errorMessages && errorMessages.length > 0) {
                  const fieldName = field as keyof ClientImageUpdateInput;
        ...
        console.log("[Component.Client.Profile.ImageUpdate] Error: ", error);
  ...
  const handleRemoveImage = async () => {
    try {
      const result = await clientImageRemoveAction();
