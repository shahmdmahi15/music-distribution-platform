...
import { ClientNameUpdateCard } from "./client-name-update-card";
import { ClientImageUpdateCard } from "./client-image-update-card";
import { ClientPasswordUpdateCard } from "./client-password-update-card";
import { ClientLinkedAccountCard } from "./client-linked-account-card";
...
export function ClientProfileView({
  ...
  const handleExportProfileSummary = () => {
    ...
    link.href = url;
    link.download = `client_profile_${user.id.slice(0, 8)}.json`;
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {activeTab === "linked" && (
        <div className="max-w-2xl">
          <ClientLinkedAccountCard linkedAccounts={linkedAccounts} />
        </div>
      )}
    </div>
  );
