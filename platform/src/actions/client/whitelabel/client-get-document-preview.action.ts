...
export async function clientGetDocumentPreviewAction(docId: string) {
  ...
  try {
    const response = await fetch(
      `${env.API_BASE_URL}/platform/client/whitelabel/documents/${docId}/preview`,
      {
        method: "GET",
        headers: {
    ...
    console.error(
      "[Action.Client.WhiteLabel.GetDocumentPreview] Error:",
      error,
    );
    return {
      success: false,
