...
export async function adminActivateWhiteLabelAction(id: string): Promise<{
  success: boolean;
  message: string;
  whiteLabel?: any;
}> {
  try {
    ...
    const res = await api.post(
      `/platform/admin/whitelabels/${id}/activate`,
      {},
      {
    ...
    if (axios.isAxiosError(error)) {
      console.error("[Action.Admin.ActivateWhiteLabel]:", {
